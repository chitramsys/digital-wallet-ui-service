...
function ProfileDetails({currentUserData, currentUserWalletData}) {
  ...
  useEffect(() => {
    ...
    if (currentUserData?.address) {
      ...
      const adrs = `${adrs1}, ${currentUserData?.address?.address2 ? currentUserData?.address?.address2 + ','  : ''} ${city}, ${state}, ${country}, ${zip}`;
      setAddress(adrs);
    }
          ...
          <div className='col-lg-2 col-md-4 col-sm-6 profile-pic-section'>
            {
              currentUserData?.profilePic ? 
                <img className='profile-image' width='200px' height='200px' alt="profile-image" src={userData?.profilePic} /> 
                : 
                <img className='profile-image' width='200px' height='200px' alt="profile-image" src={noImage} />
            }
            <span className='profile--username'>{currentUserData?.username}</span>
          </div>
          <div className='col-lg-10 col-md-8 col-sm-6'>
            <div className='details-container'>
              <div className='details-box--style2'>
                <span className='profile--label'>First Name:</span>
                <div className='profile-value--box'><span className='profile-value--text'>{userData?.name?.firstName}</span></div>
              </div>
              <div className='details-box--style2'>
                <span className='profile--label'>Middle Name:</span>
                <div className='profile-value--box'><span className='profile-value--text'>{userData?.name?.middleName}</span></div>
              </div>
            </div>
            <div className='details-container'>
              <div className='details-box--style2'>
                <span className='profile--label'>Last Name:</span>
                <div className='profile-value--box'><span className='profile-value--text'>{userData?.name?.lastName}</span></div>
              </div>
              <div className='details-box--style2'>
                <span className='profile--label'>Birth Date:</span>
                <div className='profile-value--box'><span className='profile-value--text'>{userData?.dateOfBirth}</span></div>
              </div>
            </div>
            <div className='details-box--style2'>
              <span className='profile--label'>Contact Number:</span>
              <div className='profile-value--box'><span className='profile-value--text'>{userData?.mobileNumber}</span></div>
            </div>
            <div className='details-box--style2'>
              <span className='profile--label'>Email Address:</span>
              <div className='profile-value--box'><span className='profile-value--text'>{userData?.emailAddress}</span></div>
            </div>
            <div className='details-box--style2'>
              <span className='profile--label'>Address:</span>
              <div className='profile-value--box'><span className='profile-value--text'>{address}</span></div>
            </div>
          </div>
        </div>
      </section>
