...
function LinkBankAccount() {
  ...
  const handleSubmit = (event) => {
    ...
    return (
        ...
        <label htmlFor="accountName" className="col-sm-5 col-form-label lable-align">Name on the Account</label>
            <div className="col-sm-7">
      <input type="text" minlength="5" placeholder="Enter Name on the Account"  name="accountname" value={inputs.accountname || ""} 
  ...
  <div class="mb-3 row">
      ...
      <input type="text" className="form-control" placeholder="Enter IFSC Code"  name="ifsc" value={inputs.ifsc || ""} 
        onChange={handleChange} required="true" id="ifsc"/>
    </div>
  </div>
  </div>
       
       <button className="button" style={{ backgroundColor: '#0047AB'}} type="submit"> Add</button>
       </form>
        </div>
    );
}
